...
### シーンコンテストに最初から配置されているコンポーネントへの注入

シーンに配置済みのコンポーネントへ依存を注入するためには、
コンポーネントが ```IInjectableComponent``` インターフェースを継承している必要があるので注意してください。
...
## コンテストに最初から配置されているコンポーネントへの注入

シーンやゲームオブジェクトコンテクスト以下に配置済みのコンポーネントへ依存を注入するためには、
コンポーネントが ```IInjectableComponent``` インターフェースを継承している必要があるので注意してください。
シーンのコンポーネントすべてを調査するのはコストがかかるため、このような制約をおいています。
